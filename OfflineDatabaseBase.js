 ...
 
{
		...
		var signtemp = $('input:text[id=meaning]').val();
		var videotemp = $('input:text[id=meaning]').val();
		
        db.transaction(function (tx) { tx.executeSql(insertStatement, [wordtemp, exampletemp, signexampletemp, meaningtemp], loadAndReset, signtemp, videotemp, onError); });
 ...
 
{
    ...
    db.transaction(function (tx) { tx.executeSql(updateStatement, [wordupdate, exampleupdate, signexampleupdate, meaningupdate, Number(idupdate)], loadAndReset, signupdate, videoupdate, onError); });
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
            ...
            for (var i = 0, item = null; i < dataset.length; i++) {
                ...
                var linkeditdelete = '<li>' + item['word'] + ' , ' + item['example'] + ' , ' + item['signexample'] + ' , ' + item['meaning'] + ' , ' + item['sign'] + ' , ' + item['video'] + '    ' + ' <a href="#" onclick="loadRecord(' + i + ');">edit</a>' + '    ' +
...
function showOneRecord() // Function For Retrive data from Database Display records as list
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if (palavra < dataset.length) {
                ...
                var linkeditdelete = '<li>' + item['word'] +'</li>';
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if (palavra < dataset.length) {
                ...
                var linkeditdelete = '<li>' + item['example'] + '</li>';
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if (palavra < dataset.length) {
                ...
                var linkeditdelete = '<li>' + item['signexample'] + '</li>';
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if ( palavra < dataset.length) {
               ...
               var linkeditdelete = '<li>'+ item['meaning'] + '</li>';
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if (palavra < dataset.length) {
                ...
                var linkeditdelete = '<li>' + item['sign'] + '</li>';
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
			...
			if (palavra < dataset.length) {
                ...
                var linkeditdelete = '<li>' + item['video'] + '</li>';
